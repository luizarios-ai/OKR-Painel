...
export default function CheckinDialog({ open, onOpenChange, kr, milestones }: Props) {
  ...
  async function handleSave() {
    ...
    // Invalidate all relevant caches so every view refetches fresh data from DB
    await qc.invalidateQueries({ queryKey: ["all-checkins"] });
    await qc.invalidateQueries({ queryKey: ["checkins", kr.id] });
    await qc.invalidateQueries({ queryKey: ["key_results"] });
